...
local Descs = {
    Items = {
        [Collectibles.COLLECTIBLE_SALTSHAKER] = {
            ["en_us"] = {
                Name = "",
                Desc = [[Creates a circle of salt around the player 
ERSalt
...
Mercury creep deals fire damage]],
        ...
        [Collectibles.COLLECTIBLE_SAL] = {
            ["en_us"] = {
                Name = "",
                Desc = Helpers.IDToMarkup(Collectibles.COLLECTIBLE_SALTSHAKER) ..
...
Player will shoot 4-6 salt tears in random directions#]] .. Helpers.IDToMarkup(Collectibles.COLLECTIBLE_SALTSHAKER) .. [[
...
On destroying a rock, this will shoot rock tears in random directions, these will always destroy rocks]],
        ...
        [Trinkets.TRINKET_GEODE] = {
            ["en_us"] = {
                Name = "",
                Desc = [[{{Rune}} 2.5% chance to spawn a rune when killing an enemy
{{Rune}} 25% of destroy the geode and spawn 3 runes on taking damage]],
            },
        },
        [Trinkets.TRINKET_PAPRIKA] = {
            ["en_us"] = {
                Name = "",
                Desc = [[Enemies have a 50% of explode on death
{{Burning}} Nearby enemies will get burn, will be pushed and took 25% of explotion's damage]],
            },
        },
...
-- Las funciones se resuelven una sola vez (no por cada entrada)
local addFuncs = {
...
for kind, entries in pairs(Descs) do
    local addFunc = assert(addFuncs[kind], "EID: categoria desconocida '" .. tostring(kind) .. "'")

    for id, langs in pairs(entries) do
        for lang, info in pairs(langs) do
            addFunc(EID, id, info.Desc, info.Name, lang)
        end
    end
end
